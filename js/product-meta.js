...
function buildProductRegistry() {
  ...
  if (typeof PLANS !== 'undefined') {
    PLANS.forEach(p => reg.push({
      ...
      // Product Boundaries
      entryAge: p.entryAge || '—',
      maxMaturity: p.maxMaturity || '—',
      maturityAge: p.maxMaturity || '—', // alias for UI comparison table
      minSA: p.minSA || '—',
      maxSA: p.maxSA || '—',
      premiumPay: p.premiumPay || '—',
      limitedPayTerms: p.limitedPayTerms || '—',
      premiumModes: p.premiumModes || '—',
      // Claim data
      csr: p.csr || null,
      lastUpdated: p.lastUpdated || null,
      // Core benefit
      deathBenefit: p.deathBenefit || 'Sum Assured',
      wholeLife: p.wholeLife || false,
      // Inbuilt protections
      terminalIllness: p.terminalIllness || false,
      accidentalDeath: p.accidentalDeath || false,
      criticalIllness: p.criticalIllness || false,
      wopDisability: p.wopDisability || false,
      returnOfPremium: p.returnOfPremium || false,
      // Flexibility
      premiumBreak: p.premiumBreak || false,
      smartExit: p.smartExit || false,
      lifeStage: p.lifeStage || false,
      spouseCover: p.spouseCover || false,
      jointLife: p.jointLife || false,
      // Discounts
      womenDiscount: p.womenDiscount || null,
      salariedDiscount: p.salariedDiscount || null,
      // Standard fields
      ppt: p.premiumPay || '—',
      pt: p.pt || (p.maxMaturity && p.maxMaturity !== '—' ? 'Flexible — cover up to ' + p.maxMaturity : 'Flexible — refer brochure'),
      ...
      tags: p.tags || [],
      keyStats: p.keyStats || [],
      // Derive riders and uniqueFeature from keyFeatures if not set
      riders: p.riders || (p.keyFeatures||[]).find(f=>f.toLowerCase().includes('rider')||f.toLowerCase().includes('add-on')||f.toLowerCase().includes('wop')||f.toLowerCase().includes('adb')) || (p.keyFeatures||[]).find(f=>f.toLowerCase().includes('variant')) || '—',
      uniqueFeature: p.uniqueFeature || (p.keyFeatures||[])[0] || '—',
      uniqueAdvantage: p.uniqueFeature || (p.keyFeatures||[])[0] || '—',
  ...
  if (typeof ANMOL_PLANS !== 'undefined') {
    ANMOL_PLANS.forEach(p => {
      ...
      reg.push({
        ...
        entryAge: p.entryAge || '30 days – 60 yrs',
        maturityAge: p.maturityAge || '85 yrs',
        ppt: p.isBase ? 'Single Pay | 5–12 yrs' : (p.ppt || '5–20 yrs (varies by plan option)'),
        pt: p.isBase ? 'Up to 35 yrs' : (p.pt || 'Multiple PT options — refer brochure'),
        bonusType: p.bonusType || '—',
        deathBenefit: p.isBase ? 'SA on Death + Accrued Bonuses' : (p.deathBenefit || 'SA on Death + Accrued Reversionary Bonuses + Terminal Bonus'),
        riders: p.isBase ? 'Accidental Death | CI | Surgical Care | Hospital Care' : (p.features?.riders ? '✅ Available' : '—'),
        loan: p.isBase ? true : (p.features?.loan || false),
        jointLife: p.features?.jointLife || false,
        features: p.features || {},
        uniqueFeature: p.uniqueFeature || '—',
        dataSource: p.dataSource || p.verification || '—',
  ...
  if (typeof NISHCHIT_PLANS !== 'undefined') {
    NISHCHIT_PLANS.forEach(p => {
      ...
      reg.push({
        ...
        entryAge: p.entryAge || '—',
        maturityAge: p.maturityAge || 'Depends on Policy Term chosen — refer brochure',
        ppt: p.ppt || 'Multiple PPT options — refer brochure',
        pt: p.pt || 'Multiple PT combinations — refer brochure',
        ...
        bestFor: p.uniqueFeature || '—',
        uniqueAdvantage: p.uniqueFeature || '—',
        productUrl: p.productUrl || p.calcUrl || null,
        brochureUrl: p.brochureUrl || null,
        pending: p.pending || false,
        isBase: p.isBase || false,
        keyFeatures: p.keyHighlights || p.keyFeatures || [],
  ...
  if (typeof SAVINGS_PLANS !== 'undefined') {
    SAVINGS_PLANS.forEach(p => {
      reg.push({
        ...
        type: p.type || 'Non-Linked Non-Par',
        entryAge: p.entryAge || '—',
        maturityAge: p.maturityAge ? p.maturityAge+' years' : 'Depends on Policy Term chosen',
        ppt: p.ppt || 'Multiple PPT options — refer brochure', pt: p.pt || 'Multiple PT options — refer brochure',
        deathBenefit: p.deathBenefit || '—',
        maturityBenefit: p.maturityBenefit || false,
        rop: p.rop || false,
        riders: p.riders || '—',
  ...
  if (typeof ULIP_PLANS !== 'undefined') {
    ULIP_PLANS.forEach(p => {
      reg.push({
        ...
        entryAge: p.entryAge || '—',
        maturityAge: p.maturityAge || 'Depends on PT chosen — refer brochure',
        ppt: p.ppt || 'Multiple PPT options — refer brochure', pt: p.pt || 'Multiple PT options — refer brochure',
        saMultiple: p.saMultiple || '—',
        deathBenefit: p.deathBenefit || 'Higher of Fund Value or Sum Assured (min. 105% of total premiums paid) — refer brochure',
  ...
  if (typeof INCOME_PLANS !== 'undefined') {
    INCOME_PLANS.forEach(p => {
      reg.push({
        ...
        company: p.company, plan: p.plan, uin: p.uin || '—',
        type: p.type || 'Non-Linked Non-Par',
        entryAge: p.entryAge || '—', maturityAge: p.maturityAge || 'Depends on PT chosen — refer brochure',
        ppt: p.ppt || 'Multiple PPT options — refer brochure', pt: p.pt || 'Multiple PT options — refer brochure',
        ...
        guaranteedAdditions: p.guaranteedAdditions || false,
        loyaltyAdditions: p.loyaltyAdditions || false,
        incomeBenefit: p.incomeBenefit || true,
        incomePeriods: p.incomePeriods || [],
        planOptions: p.planOptions || [],
        commutation: p.commutation || false,
        incomePayout: p.incomePayout || 'Annual / Half-Yearly / Quarterly / Monthly',
        incomeAfterDeath: p.incomeAfterDeath || false,
        increasingIncome: p.increasingIncome || false,
        incomePeriod: p.incomePeriods ? p.incomePeriods.join(' / ') : '—',
        incomeFrequency: p.incomePayout || 'Annual / Half-Yearly / Quarterly / Monthly',
        uniqueFeature: p.uniqueFeature || '—',
        pitch: p.pitch || '—',
        bestFor: p.uniqueFeature || '—',
        uniqueAdvantage: p.uniqueFeature || '—',
  ...
  if (typeof ANNUITY_PLANS !== 'undefined') {
    ANNUITY_PLANS.forEach(p => {
      reg.push({
        ...
        entryAge: p.entryAge || '—', maturityAge: p.maturityAge || 'Not applicable — annuity pays for life',
        ppt: p.limitedPayNote || (p.limitedPay ? 'Limited Pay available' : 'Single Pay'),
        pt: p.type || '—',
        deathBenefit: 'As per annuity option selected',
        riders: p.riders || 'Not typically applicable — annuity plans generally have no riders',
        loan: typeof p.loan === 'string' ? true : (p.loan || false),
        jointLife: p.jointLife || false,
        totalOptions: p.totalOptions || '—',
        minPremium: p.minPremium || '—',
        payout: p.payout || '—',
        limitedPay: p.limitedPay || false,
        limitedPayNote: p.limitedPayNote || '—',
        qrops: p.qrops || false,
        topUp: p.topUp || false,
        groupPolicy: p.groupPolicy || false,
        uniqueFeature: p.uniqueFeature || getMeta(p.isBase ? 'absli-ann' : 'ann_'+p.id)?.bestFor || '—',
        bestFor: p.uniqueFeature || getMeta(p.isBase ? 'absli-ann' : 'ann_'+p.id)?.bestFor || '—',
        uniqueAdvantage: p.uniqueFeature || getMeta(p.isBase ? 'absli-ann' : 'ann_'+p.id)?.bestFor || '—',
