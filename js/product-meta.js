  ...
  if (typeof ANMOL_PLANS !== 'undefined') {
    ANMOL_PLANS.forEach(p => {
      ...
      reg.push({
        ...
        deathBenefit: p.isBase ? 'SA on Death + Accrued Bonuses' : (p.deathBenefit || '—'),
  ...
  if (typeof NISHCHIT_PLANS !== 'undefined') {
    NISHCHIT_PLANS.forEach(p => {
      ...
      reg.push({
        ...
        deathBenefit: p.deathBenefit || (p.isBase ? 'SA on Death (10× AP)' : '—'),
        riders: p.riders || '—', loan: p.loan || false, jointLife: false,
        guaranteedAdditions: false, loyaltyAdditions: false,
        incomeBenefit: true,
  ...
  if (typeof SAVINGS_PLANS !== 'undefined') {
    SAVINGS_PLANS.forEach(p => {
      reg.push({
        ...
        type: p.type || 'Non-Linked Non-Par',
        entryAge: p.entryAge || '—',
        maturityAge: p.maturityAge ? p.maturityAge+' years' : '—',
        ppt: p.ppt || '—', pt: p.pt || '—',
